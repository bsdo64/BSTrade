...
class OrderBookModel(QObject):
    ...
    def slt_ws_message(self, msg):
        ...
        action = j.get('action')
        if table_name in ['orderBookL2', 'orderBookL2_25']:
            items = j.get('data')
            if action == 'partial':
    ...
    def update_view(self):
        ...
        if bound_idx > -1:
            ...
            self.sell_view.set_data(self.book[max(b_idx - depth, 0):b_idx])
            self.buy_view.set_data(self.book[b_idx:min(b_idx + depth, len(self.book))])
